...
def evaluate():
    global calculation
    try:
        calculation=str(eval(calculation))#aqui es donde se realiza la operacion
        text_result.delete(1.0,"end")
        text_result.insert(1.0, calculation)
    except:
        clear()
...
root =tk.Tk()# objeto ventana
root.geometry("300x275")#tamaño ventana

text_result = tk.Text(root, height=2, width=16, font=("Arial",24))#define ventana del texto, altura y ancho y fuente
text_result.grid(columnspan=5)#coloca el texto en la ventana principal, ventana ocupa 5 cols
...
root.mainloop()#visualiza la ventana